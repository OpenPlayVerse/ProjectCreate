...
do --arg parsing 
	...
	parser:option("-T --target --branch", "Target branch"):count(1):args(1):target("target")
	...
	parser:option("-R --release-folder", "Folder in wich release folder/files are stored and released seperately"):count("*"):target("releaseFolders")
	parser:flag("--release-files-only", "Stores the files from the release filders instead of the folders itself."):target("filesOnly")
...
local function processFile(pathArg)
	...
	if not uploadedFiles[file] then
		if fileType == "directory" then
			...
			lfs.chdir(path)
			if args.filesOnly then
				lfs.chdir(dirName)
				os.execute("zip -r ../$tmpReleaseFileLocation * .*")
				lfs.chdir("..")
			else
